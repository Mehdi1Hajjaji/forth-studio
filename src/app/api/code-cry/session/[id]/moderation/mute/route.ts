...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    await (prisma as any).codeCryMute.upsert({
    ...
    return jsonError('Failed to mute user.', code, e?.message ?? String(e));
  }
}
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    await (prisma as any).codeCryMute.delete({ where: { sessionId_userId: { sessionId: params.id, userId } } });
    return NextResponse.json({ ok: true });
  } catch (e: any) {
    const code = e?.message === '401' ? 401 : e?.message === '403' ? 403 : e?.message === '404' ? 404 : 400;
    return jsonError('Failed to unmute user.', code, e?.message ?? String(e));
  }
}
